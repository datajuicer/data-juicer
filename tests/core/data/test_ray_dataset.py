...
class RayDatasetFuncsTest(DataJuicerTestCaseBase):
    ...
    @TEST_TAG('ray')
    def test_set_dataset_to_absolute_path(self):
        """Test set_dataset_to_absolute_path function"""
        import tempfile
        import os
        
        # Create temporary directory for testing
        with tempfile.TemporaryDirectory() as tmpdir:
            # Create a config with image/video/audio keys
            ...
            # Get results
            result_data = result_dataset.take_all()
            ...
            for item in result_data:
                ...
                for aud_path in item.get('audios', []):
                    self.assertTrue(os.path.isabs(aud_path) or aud_path.startswith('s3://'))

    @TEST_TAG('ray')
    def test_preprocess_dataset(self):
        """Test preprocess_dataset function"""
        import tempfile
        import os
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Verify the dataset was processed (paths should be absolute now)
            result_data = result_dataset.take_all()
