...
from data_juicer.ops.mapper.video_captioning_face_attribute_emotion_mapper import (
...
class VideoCaptioningFaceAttributeEmotionMapperTest(DataJuicerTestCaseBase):
    """Test for video_captioning_face_attribute_emotion_mapper, which
    generates facial attribute and emotion descriptions for each tracked
    person using VideoLLaMA3.
    ...
    def _run_mapper(self, ds_list, op, num_proc=1):
        ...
        res = dataset.flatten().select_columns([
            f'{Fields.meta}.{MetaKeys.video_facetrack_attribute_emotion}',
        ])
        return res[f'{Fields.meta}.{MetaKeys.video_facetrack_attribute_emotion}']

    def test_single_video(self):
        """Face attribute emotion should produce a description for each track."""
        ...
        op = VideoCaptioningFaceAttributeEmotionMapper(
            video_describe_model_path=hf_model,
            cropping_face_video_temp_path=self._temp_dir,
