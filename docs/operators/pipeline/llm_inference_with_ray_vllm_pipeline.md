...
Use `llm_ray_vllm_engine_pipeline` as the operator name in YAML recipes. This registered name differs from the source filename.

YAML 菜谱中的算子名为 `llm_ray_vllm_engine_pipeline`，与源码文件名不同。
