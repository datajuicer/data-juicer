...
class AdaptiveBatchContractError(ValueError):
    """The mapper violated the row-preserving microbatch contract."""
...
def _slice_batch(batch, start: int, end: int, total_rows: int):
    ...
    return deepcopy(batch[start:end])
...
def _merge_outputs(outputs: List[Any]):
    ...
    if isinstance(first, Mapping):
        ...
        if any(set(output) != expected_keys for output in outputs):
            raise AdaptiveBatchContractError("mapper output schema changed across micro-batches")
        merged = {}
        for key in keys:
            ...
            if isinstance(first_value, (str, bytes)):
                if any(value != first_value for value in values[1:]):
                    raise AdaptiveBatchContractError(f"scalar output field {key!r} changed across micro-batches")
...
class OOMSafeAdaptiveMapper:
    ...
    def __init__(
        ...
        if max_floor_retries < 0:
            raise ValueError("max_floor_retries must be non-negative")
        if floor_retry_backoff_sec < 0:
            raise ValueError("floor_retry_backoff_sec must be non-negative")
        self.mapper = mapper
    ...
    def __call__(self, batch, *args, **kwargs):
        ...
        # Cost-aware controllers get a per-row cost observation for the
        # incoming block before any slicing decision. Best-effort: a missing
        # cost column must never break processing.
        if self.batch_cost_fn is not None:
            observe_cost = getattr(self.controller, "observe_batch_cost", None)
            if observe_cost is not None:
                try:
                    observe_cost(float(self.batch_cost_fn(batch)))
                except Exception:  # noqa: BLE001 - diagnostics must not fail the pipeline
                    pass
            ...
            while True:
                ...
                try:
                    ...
                    try:
                        ...
                        # Transient co-location starvation: a sibling actor
                        # may be holding a huge slice right now (fractional-
                        # GPU packing).  Bounded same-size retries give it a
                        # chance to release before the floor becomes fatal;
                        # the slice, size and error stay identical (same-
                        # slice semantics) and the propagation after the
                        # budget is exhausted stays explicit.
                        ...
                        logger.error(
                            ...
                            f"({batch_size}) persisted after {self.max_floor_retries} "
                            f"bounded wait-retr(ies); propagating (no smaller slice possible)"
                        )
                        self._emit_snapshot(measurement)
                        raise error
                ...
                if self.validate_output_rows:
                    try:
                        ...
                        raise AdaptiveBatchContractError("mapper returned an invalid column batch") from error
                    if output_rows != batch_size:
                        self._emit_snapshot(measurement)
                        raise AdaptiveBatchContractError(
                            f"mapper returned {output_rows} rows for an {batch_size}-row input micro-batch"
                        )
                if floor_retries and batch_size == self.controller.min_batch_size:
                    record_recovery = getattr(self.controller, "record_floor_recovery", None)
