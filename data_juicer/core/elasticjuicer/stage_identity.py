...
def _normalize(value):
    ...
    if hasattr(value, "_init_kwargs"):
        return {"operator_fingerprint": operator_fingerprint(value)}
    return repr(value)
...
def operator_fingerprint(op) -> str:
    ...
    payload = {
        ...
        "args": _normalize(getattr(op, "_init_args", ())),
        "kwargs": _normalize(
            {key: value for key, value in (getattr(op, "_init_kwargs", None) or {}).items() if key != "work_dir"}
        ),
    }
...
def assign_stage_identities(ops) -> Dict:
    ...
    before resource probing or checkpoint grouping) so indices and occurrences
    are stable across fresh runs and checkpoint resumes of the same recipe.
    """
