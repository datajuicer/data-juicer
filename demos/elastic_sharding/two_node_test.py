#!/usr/bin/env python3
"""Small two-node integration test for the elastic sharding demo.

For DLC, submit one ``dlc`` command as a two-worker job. Every worker runs the
same entry point; shared-storage leader election coordinates preparation and
final verification without depending on platform-specific rank variables.
...
def worker(args: argparse.Namespace) -> int:
    print(
        f"Starting test worker on hostname={socket.gethostname()}; "
        f"this invocation will handle at most {args.max_shards} shards.",
        flush=True,
    )
    command = [
        "worker",
...
def dlc(args: argparse.Namespace) -> int:
    """Run the complete smoke test from every DLC worker instance."""
    ...
    if (args.nodes - 1) * max_shards >= args.num_shards:
        raise ValueError(
            "This shard/node combination cannot guarantee that every DLC node "
            "participates. Increase --num-shards (4 shards for 2 nodes is the "
            "recommended smoke test)."
        )

    print(
        f"DLC instance hostname={socket.gethostname()} starting; "
        f"nodes={args.nodes}, shards={args.num_shards}, "
        f"max_shards_per_node={max_shards}.",
        flush=True,
    )
...
def verify(args: argparse.Namespace) -> int:
    ...
    output_path = Path(args.output).expanduser().resolve() if args.output else job_dir / "two-node-merged.jsonl"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare_parser = commands.add_parser("prepare", help="Split input into four test shards")
    ...
    dlc_parser = commands.add_parser(
        "dlc",
        help="Run the complete test as one multi-worker DLC command",
    )
    ...
    dlc_parser.add_argument(
        ...
        type=_positive_int,
        default=2,
        help="Expected number of DLC Worker instances (default: 2)",
    )
    ...
    dlc_parser.add_argument(
        "--output",
        help="Merged output; defaults to two-node-merged.jsonl in the job directory",
    )
    dlc_parser.add_argument(
        "--wait-timeout-secs",
    ...
    worker_parser = commands.add_parser("worker", help="Run on each of the two nodes")
    worker_parser.add_argument("--job-dir", required=True, help="Shared job directory")
    worker_parser.add_argument(
        "--max-shards",
        type=_positive_int,
        default=2,
        help="Keep the default 2 so both nodes must participate in a four-shard test",
    )
    worker_parser.add_argument(
        "--ray-address",
    ...
    verify_parser = commands.add_parser("verify", help="Check two-node ownership and merge")
    ...
    verify_parser.add_argument("--expect-nodes", type=_positive_int, default=2)
