#!/usr/bin/env python3
"""Durable shared-filesystem shard jobs used by Data-Juicer executors.

Each shard is processed by an isolated ``ray`` executor on the claiming node.
This module owns the platform-neutral preparation, claim, retry, validation,
and merge state machine. Scheduler-specific launchers remain outside core.
"""
...
def prepare_job(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "heartbeat_interval_secs", DEFAULT_HEARTBEAT_INTERVAL_SECS) >= args.lock_timeout_secs:
        raise ShardJobError("--heartbeat-interval-secs must be less than --lock-timeout-secs")
    ...
    try:
        ...
        manifest = {
            ...
            "policy": {
                ...
                "heartbeat_interval_secs": getattr(args, "heartbeat_interval_secs", DEFAULT_HEARTBEAT_INTERVAL_SECS),
...
def _refresh_lock(lock_path: Path, token: str) -> bool:
    """Refresh a claim's mtime only while ``token`` still owns the lock."""
    try:
        fd = os.open(lock_path, os.O_RDONLY)
    except FileNotFoundError:
        return False
...
def _heartbeat_lock(lock_path: Path, token: str, interval_secs: int, stop: threading.Event) -> None:
    while not stop.wait(interval_secs):
        if not _refresh_lock(lock_path, token):
            return
...
def _process_claim(
    ...
    heartbeat = threading.Thread(
        ...
        args=(
            lock_path,
            token,
            int(manifest.get("policy", {}).get("heartbeat_interval_secs", DEFAULT_HEARTBEAT_INTERVAL_SECS)),
            heartbeat_stop,
        ),
        name=f"elastic-shard-heartbeat-{shard_id}",
        daemon=True,
    )
    heartbeat.start()
    try:
    ...
    if validation is not None:
        done_metadata = {
            ...
            **validation,
            # The worker commit was fenced by ``_current_commit_matches``
            # before claims started, so reuse the immutable manifest value.
            "data_juicer_commit": manifest.get("data_juicer", {}).get("commit", "unknown"),
        }
...
def _effective_policy(manifest: dict[str, Any], args: argparse.Namespace) -> dict[str, int]:
    ...
    policy = {
        ...
        "heartbeat_interval_secs": int(
            ...
            if getattr(args, "heartbeat_interval_secs", None) is not None
            else saved.get("heartbeat_interval_secs", DEFAULT_HEARTBEAT_INTERVAL_SECS)
        ),
        "max_retries": int(
            args.max_retries
    ...
    if policy["heartbeat_interval_secs"] >= policy["lock_timeout_secs"]:
        raise ShardJobError("heartbeat_interval_secs must be less than lock_timeout_secs")
    return policy
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare.add_argument("--heartbeat-interval-secs", type=_positive_int, default=DEFAULT_HEARTBEAT_INTERVAL_SECS)
    ...
    worker.add_argument("--heartbeat-interval-secs", type=_positive_int)
